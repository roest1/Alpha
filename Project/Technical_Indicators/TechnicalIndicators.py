...
class TechnicalIndicators:
    def __init__(self, df):
        self.df = df.copy()
        self.df = self.bollinger_bands_signal_generation()
        self.df = self.dual_thrust_signal_generation()
        self.df = self.heikin_ashi_signal_generation()
        self.df = self.awesome_signal_generation()
    ...
    def bollinger_bands_signal_generation(self, window=20, num_of_std=2):
        df = self.df
        df['bollinger bands std'] = df['Close'].rolling(
            window=window, min_periods=20).std()
        ...
        df['bollinger bands upper band'] = df['bollinger bands mid band'] + \
            num_of_std*df['bollinger bands std']
    ...
    def dual_thrust_signal_generation(self, window=5, trigger_range=0.5):
        df = self.df
    ...
    def awesome_signal_generation(self, ma1=5, ma2=34):
        df = self.df
    ...
    def macd_signal_generation(self, ma1=5, ma2=34):
        df = self.df
    ...
    def parabolic_sar_signal_generation(self):
        df = self.df
        initial_af = 0.02
        step_af = 0.02
        end_af = 0.2
    ...
    def rsi_signal_generation(self, lag_days=14):
        df = self.df
        """
        smma()
