...
Engle-Granger two-step method to test cointegration

A more common test to check cointegration is johansen test (sm.tsa.var.vecm.coint_johansen)

Unlike the johansen test, two-step method can only detect first order
"""
...
Verify status of cointegration by checking historical data

bandwidth : # data points for consideration

If status is valid, we check the signals when z stat is above the upper bound
...
Function to visualize position
"""
...
def main():
    ...
    portfolio_details.to_csv('pair_trading_portfolio.csv')
